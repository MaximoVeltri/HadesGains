...
import { FaLeaf, FaShieldAlt, FaCheck } from 'react-icons/fa';
// Importamos el hook personalizado para el carrito
...
const packs = [
  ...
  { id: 3, title: "Popular", amount: "Pack x6", price: 15.99, featured: true }, // Pack destacado
...
function Product() {
  ...
  const fadeIn = {
    hidden: { opacity: 0, y: 20 }, // Estado inicial: invisible y 20px abajo
    visible: { opacity: 1, y: 0 }  // Estado final: visible y en posición
  };
        ...
        >
          ...
          <motion.div variants={fadeIn} className="grid md:grid-cols-2 gap-12 items-center mb-20">
            <div className="space-y-6">
              ...
              {/* Sección de precios y descuento */}
            ...
            {/* Contenedor de la imagen del producto con animación */}
            <motion.div
              className="relative"
              whileHover={{ scale: 1.05 }}
              ...
              {packs.map((pack) => (
                <motion.div
                  ...
                  className={`p-8 rounded-xl ${
                    pack.featured 
                      ? 'bg-primary text-white scale-105' // Estilo especial para pack destacado
                      : 'bg-white'
                  } shadow-xl`}
                  whileHover={{ y: -10 }}
                >
